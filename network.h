...
static void* get_opponent_input(void *arg) {
    ...
    while (*game_stat->status==RUNNING) {
        if (!myturn) {
            rc=read(client_socket_fd, *(game_stat->op_r), sizeof(int));
            if (rc==-1 || rc==0) {
            ...
            rc=read(client_socket_fd, *(game_stat->op_c), sizeof(int));
            if (rc==-1 || rc==0) {
                close(*game_stat->client_fd);
                *game_stat->status==WAITING;
                break;
            }
            rc=read(client_socket_fd, *(game_stat->status), sizeof(int));
            if (rc==-1 || rc==0) {
                close(*game_stat->client_fd);
                *game_stat->status==WAITING;
                break;
